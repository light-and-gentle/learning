	...
	$.ajax({
		...
		success : function(result) { 
			...
			for ( var number in result.list) {
				content+='<tr><td onclick="showPDF(\''+result.list[number].url+'\')">'+result.list[number].title+'</td><td>'+result.list[number].time+'</td><td>'+result.list[number].stateText+'</td><td>'+result.list[number].typeText+'</td><td> <span class="send-btn" title="发布"  onclick="updateProductState(3,\'\','+result.list[number].id+')"></span><span class="del-btn" title="删除"  onclick="deleteProduct('+result.list[number].id+')"></span><span class="pass-btn" title="不通过" onclick="ONPass('+result.list[number].id+')"></span></td></tr>';
			}
			if(content==""){
